...
LOG_MODULE_DECLARE(vcnl4035, CONFIG_SENSOR_LOG_LEVEL);

static void vcnl4035_handle_cb(struct vcnl4035_data *data)
{
	const struct vcnl4035_config *config = data->dev->config;

	gpio_pin_interrupt_configure_dt(&config->int_gpio, GPIO_INT_DISABLE);

#if defined(CONFIG_VCNL4035_TRIGGER_OWN_THREAD)
	k_sem_give(&data->trig_sem);
#elif defined(CONFIG_VCNL4035_TRIGGER_GLOBAL_THREAD)
	k_work_submit(&data->work);
#endif
}

static void vcnl4035_gpio_callback(const struct device *dev,
				   struct gpio_callback *cb,
				   uint32_t pin_mask)
{
	struct vcnl4035_data *data =
		CONTAINER_OF(cb, struct vcnl4035_data, gpio_cb);
	const struct vcnl4035_config *config = data->dev->config;
	...
	vcnl4035_handle_cb(data);
}

static int vcnl4035_handle_proxy_int(const struct device *dev)
{
...
static int vcnl4035_handle_als_int(const struct device *dev)
{
	struct vcnl4035_data *data = dev->data;
...
static void vcnl4035_handle_int(const struct device *dev)
{
	...
	if (vcnl4035_read(dev, VCNL4035_REG_INT_FLAG, &int_source)) {
	...
	if (data->int_type == VCNL4035_PROXIMITY_AWAY ||
	    data->int_type == VCNL4035_PROXIMITY_CLOSE) {
		vcnl4035_handle_proxy_int(dev);
	} else if (data->int_type == VCNL4035_AMBIENT_HIGH ||
		   data->int_type == VCNL4035_AMBIENT_LOW) {
		vcnl4035_handle_als_int(dev);
	} else {
...
static void vcnl4035_thread_main(struct vcnl4035_data *data)
{
	while (true) {
		k_sem_take(&data->trig_sem, K_FOREVER);
		vcnl4035_handle_int(data->dev);
	}
}
#endif

#ifdef CONFIG_VCNL4035_TRIGGER_GLOBAL_THREAD
static void vcnl4035_work_handler(struct k_work *work)
{
	struct vcnl4035_data *data =
		CONTAINER_OF(work, struct vcnl4035_data, work);
		      ...
		      const struct sensor_value *val)
{
	...
	if (chan == SENSOR_CHAN_PROX) {
		if (attr == SENSOR_ATTR_UPPER_THRESH) {
			if (vcnl4035_write(dev, VCNL4035_REG_PS_THDH,
					   (uint16_t)val->val1)) {
				ret = -EIO;
				goto exit;
			}
		...
		if (attr == SENSOR_ATTR_LOWER_THRESH) {
			if (vcnl4035_write(dev, VCNL4035_REG_PS_THDL,
					   (uint16_t)val->val1)) {
				ret = -EIO;
				goto exit;
			}
	...
	if (chan == SENSOR_CHAN_LIGHT) {
		if (attr == SENSOR_ATTR_UPPER_THRESH) {
			if (vcnl4035_write(dev, VCNL4035_REG_ALS_THDH,
					   (uint16_t)val->val1)) {
				ret = -EIO;
				goto exit;
			}
		...
		if (attr == SENSOR_ATTR_LOWER_THRESH) {
			if (vcnl4035_write(dev, VCNL4035_REG_ALS_THDL,
					   (uint16_t)val->val1)) {
				ret = -EIO;
				goto exit;
			}
...
int vcnl4035_trigger_init(const struct device *dev)
{
	...
	k_thread_create(&data->thread, data->thread_stack,
			CONFIG_VCNL4035_THREAD_STACK_SIZE,
			(k_thread_entry_t)vcnl4035_thread_main,
			data, NULL, NULL,
			K_PRIO_COOP(CONFIG_VCNL4035_THREAD_PRIORITY),
			0, K_NO_WAIT);
	k_thread_name_set(&data->thread, "VCNL4035 trigger");
#elif defined(CONFIG_VCNL4035_TRIGGER_GLOBAL_THREAD)
	data->work.handler = vcnl4035_work_handler;
#endif

	gpio_pin_configure_dt(&config->int_gpio, GPIO_INPUT);

	gpio_init_callback(&data->gpio_cb, vcnl4035_gpio_callback,
